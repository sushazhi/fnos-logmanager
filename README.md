...
- **UI 设计** 
  - 鸿蒙 7.0 Liquid Glass（液态玻璃）设计语言，引入完整设计令牌系统 (Design Token System)
  - 全局 CSS 变量色彩体系（主题色 / 渐变 / 统计卡片色系 / 背景色）
  - 空间化 3D 景深阴影 (Spatial Depth Shadows)
  - 统一过渡曲线系统 (Motion Curves) 与动态光效 (Dynamic Glow)
...
### v0.8.0
- **鸿蒙 7.0 液态玻璃 (Liquid Glass) 设计体系全面重构**：引入完整设计令牌系统、空间化 3D 景深阴影、统一过渡曲线、动态光效、聚焦环与空间化变换。
- **全组件视觉升级**：卡片 / 按钮 / 对话框 / 面板统一玻璃拟态与光影效果，深色与夜间主题重制。
- **通知面板重构**：合并分散的子组件为统一单文件结构，交互更流畅一致。
- 新增鸿蒙 7.0 轻提示 (Toast) 与日志「复制可见区域」按钮。
- 新增 Linux 内核版本管理功能。
- 通知规则支持 IP 白名单（白名单来源的登录事件跳过推送）。
- 修复钉钉配置键映射、网关认证 Header、移动网络认证回退等若干问题。
